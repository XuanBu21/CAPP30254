...
def temporal_validation(df, predictors, target, time_var, start_time, end_time):
    '''
    ...
    Inputs:
        ...
        start_time: (object) first time stamp to split data
        end_time: (object) second time stamp to split data
    Returns:
        dataframes of training data and testing data
    '''
    x_set = df[predictors]
    y_set = df[target]
    train_data = x_set[df[time_var] <= start_time]
    test_data = x_set[(start_time < df[time_var]) & (df[time_var]< end_time)]
    train_target = y_set[df[time_var] <= start_time]
    test_target = y_set[(start_time < df[time_var]) & (df[time_var]< end_time)]
    return train_data, test_data, train_target, test_target
...
def evaluate_classifier(best_grid, train_data, test_data, train_target, test_target):
    ...
    # evaluate performances of classifiers under different thresholds
        for threshold in THRESHOLD:
            ...
            a_at_threshold = 'a_at_threshold_' + str(threshold)
            ...
            models[name][a_at_threshold] = accuracy_score(test_target, pred_label)
